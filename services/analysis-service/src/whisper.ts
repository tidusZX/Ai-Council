 ...
 * Whisper auto-extracts audio from common video containers (mp4, webm, mov).
 */
export async function transcribe(filePath: string): Promise<string> {
  const stat = statSync(filePath)
  if (stat.size > MAX_FILE_BYTES) {
    throw new Error(
      `File ${filePath} is ${Math.round(stat.size / 1024 / 1024)}MB; Whisper API max is 25MB. Add audio extraction step.`
    )
  }
